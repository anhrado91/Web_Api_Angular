	...
	$.fn.tagEditor = function (options, val, blur) {
		...
		if (typeof options == 'string') {
			...
			selector.each(function () {
				...
				if (options == 'getTags')
					...
					if (!blur) ed.click();
				} else if (options == 'editTag') {
					// trigger delete on matching tag, then click editor to create a new tag
					$('.tag-editor-tag', ed).filter(function () { return $(this).text() == val; }).click();
					//if (!blur) ed.click();
		...
		return selector.each(function () {
			...
			function set_placeholder() {
			    ed.find('.placeholder').remove();
			    if (o.placeholder && !ed.find('.tag-editor-tag.active').length) {
